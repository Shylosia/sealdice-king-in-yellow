...
if (!ext) {
    ext = seal.ext.new('KingInYellowSuite', '巫洁鸾', '1.0.0');
    seal.ext.register(ext);
}

//图片请存放在 data/images/KingInYellow，如果不行，请注释下一行，改用全局路径
const BASE_PATH = "KingInYellow";
//上方路径无法识别或读取，改用下方的全局路径，直接复制文件地址粘贴到此处
//const BASE_PATH = "C:\\data\\images\\KingInYellow";
...
// === 文档部分 (包含图片路径) ===
const POS_IMG = `\n\n[CQ:image,file=${BASE_PATH}\\Pos.png]`;
...
Finally, the medium interprets the reading.${POS_IMG}`;
...
cmdKas.solve = function (ctx, msg, argv) {
    ...
    if (!isDangerMode) {
        ...
        output += `${getCardImage(EMPEROR_ID, false)}\n`;
