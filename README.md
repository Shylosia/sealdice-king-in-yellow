这是一个为您编写的 README.md 文档。根据您的要求，我特别强调了版权和图片占位符的问题，并详细说明了文件结构配置，以便用户能够顺利部署。

黄衣之王 (King in Yellow) - SealDice JavaScript 插件
“王在王座上端坐，笑看闹剧。”

本插件为 SealDice 骰子机器人实现了 “王与杖” (King and Staff) 塔罗牌阵抽取功能。该牌阵源自 Arc Dream Publishing 出版的《黄衣之王》塔罗牌（The King in Yellow Tarot），用于模拟一种诡谲、戏剧性的占卜体验。

此外，本插件还集成了通用的塔罗牌阵（如凯尔特十字、圣三角等），支持中英双语输出。
...
图片占位符：插件代码中预设了图片读取路径，但在您未手动配置图片前，骰娘将无法发送卡面图片（或显示为空白/错误）。
...
📂 安装与配置
1. 安装脚本
将 King and Staff.js 放入您的 SealDice data/scripts 目录（或您使用的加载器的相应目录），并在控制台重载 js 脚本。
...
.kas danger 或 .kas 危 - 皇帝 (The Emperor) 入局。
...
实现了独特的“不可名状之牌” (The Ineffable) 逻辑：即使未被抽出，也会在最后覆盖在指示物上。
